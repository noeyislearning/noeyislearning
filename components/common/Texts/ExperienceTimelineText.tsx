      ...
      <div className="flex flex-col gap-4">
        <ol className="relative border-l border-gray-200 dark:border-gray-700">                  
          <li className={last ? "ml-4 flex flex-col" : "mb-10 ml-4 flex flex-col"}>
            <div className="absolute w-3 h-3 bg-gray-700 rounded-full mt-1.5 -left-1.5 border border-gray-700"></div>
            <div className="flex flex-col gap-1">
              ...
              {exp.skills.map((skill, index) => (
                <>
                  <p className="text-xs" key={skill}>{skill}</p>
                  {index !== exp.skills.length - 1 && <div>•</div>}
                </>
              ))}
              </div>
            </div>
