...
export default function WorkCard({ work } : WorkCardProps) {
  ...
  return (
    <Link href={work.redirect_url} target={"_blank"} className="group bg-indigo-800 w-full h-full relative"> 
      <div className="absolute -top-6 left-0 m-2 flex flex-row gap-2 items-center">
        { work.is_ongoing && (
      ...
      <div className="p-4 w-full flex flex-col gap-2 items-start divide-y divide-dashed divide-white border border-dashed rounded-lg hover:border-yellow-400">      
        ...
        <div className="pt-4 w-full flex flex-col gap-2 relative">
          <Image src={work.img_url} alt="GitHub History" width={1000} height={1000} className="object-cover w-full h-48 rounded-lg grayscale group-hover:grayscale-0 duration-300 transition-all ease-in-out"/>
